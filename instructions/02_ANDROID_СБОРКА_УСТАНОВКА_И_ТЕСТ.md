...
- В `Settings -> Execution timing` лимит smart-enter/smart-exit задаётся в минутах от `1` до `30`; backend по умолчанию предлагает `10` минут.
- `Until filled (max 30 min)` продолжает попытки, пока объём не исполнен полностью, но всегда останавливается через 30 минут.
