...
> Статус с 2026-08-13: автономные auto-rebalance и emergency de-risk,
> сокращающие позиции, удалены из production-кода и не должны запускаться.
> Активными остаются защитные stop/take, контроль маржи и проверенная очистка
> защитных ордеров без позиции. Ниже сохранена историческая информация об
> экспериментах; она не является инструкцией на запуск автономного сокращения.
...
## Безопасная замена KuCoin stop/take после инцидента TUT

- Возраст exchange-native stop больше не является причиной ротации:
  `protective.stop_force_requote_max_age_sec=0`. Обновление требуется только
  при реальном расхождении цены, количества или стороны.
- Для KuCoin действует порядок `create -> fresh verify -> cancel old -> fresh
  verify`. Старый stop/take нельзя отменять, пока новый не виден в private
  open-stop snapshot и не совпадает с целевой защитой.
- Если новый ордер не появился, старый остаётся активным, а цикл возвращает
  `replacement_unverified_old_kept`. Ошибка отмены после подтверждённой замены
  возвращается отдельно и не маскируется как успешное обновление.
- Private KuCoin order stream распознаёт критическое событие
  `protective_stop_zero_fill`, когда `stopTriggered=true`, но `filledSize=0`.
  В событии сохраняются symbol/order/stop/remark без секретов; это сигнал для
  немедленной диагностики, но не разрешение на слепой повторный market order.
- Эти правила появились после TUT 2026-08-09: старый stop был отменён примерно
  на 49,5 секунды, новый stop сработал без контрагента в price-protection
  диапазоне, а ликвидация последовала примерно через одну секунду.
