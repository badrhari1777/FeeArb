...
This is a reproducible research-only comparison for a `$3000` Pump portfolio.
It does not change live sizing, margin, orders, transfer limits, or the active
risk policy. Reproduce it with:
...
The post-fill/next-step model requires roughly `$195` total tracked extra
margin before L3 is allowed to remain live, versus `$105` now. The theoretical
increment is about `$90`; production must round upward and correct against a
fresh exchange reread. At `$195`, both the old pre-fill stop and projected
...
| safe `$525`, projected gate | 35 | 173.36% | 3.15% | 0 | 0 | `$1180` | `$355` | 300 h |
...
fills, and needed at most `$355` from main in the conservative replay. A fresh
transfer preflight showed `$1119.93` on main; the existing `$500` main floor
leaves only about `$619.93` immediately usable, enough for this candidate but
not the `$600` policy's `$820` historical peak.
...
## Limitations before a live policy change
