...
BITGET_TRADE_WS_URL = "wss://ws.bitget.com/v2/ws/private"

logger = logging.getLogger(__name__)
_RAW_LOGGER = logging.getLogger("ws_trade_bitget_trade_raw")
if not _RAW_LOGGER.handlers:
    _RAW_LOGGER.setLevel(logging.INFO)
    log_path = BASE_DIR / "logs" / "ws_trade_bitget_trade_raw.log"
...
def _bitget_gateway() -> ExchangeGateway:
    for spec in EXCHANGE_SPECS:
        if spec.slug == "bitget":
            return ExchangeGateway(spec)
    raise RuntimeError("bitget spec not found")


class WsTradeBitgetTradeRawStream:
    ...
    async def _connect(self) -> None:
        ...
        await self._log_line("[sys] connected to bitget trade ws")
        await self._auth_bitget()

    async def _auth_bitget(self) -> None:
        if not self._remote_ws:
            return
        _bootstrap_env(force=True)
        gateway = _bitget_gateway()
        await gateway.refresh_credentials_async(force_env=True)
        api_key = gateway.api_key
        api_secret = gateway.api_secret
        passphrase = gateway.password
        if not api_key or not api_secret or not passphrase:
            await self._log_line("[err] bitget api key/secret/passphrase missing")
            return
